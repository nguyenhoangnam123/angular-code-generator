...
export function addRouteToAppRoutingModule(
  ...
  const { name, flat } = options;
  const path = strings.dasherize(name);
  const modulePath = options.module;
  console.log(`module path: ${modulePath}`);
  const childrenPath = flat
    ? normalize('./' + constants.viewsFolder)
    : normalize('./' + constants.viewsFolder + '/' + strings.dasherize(name));
  const loadChildren =
    'app/' +
    normalize(
      childrenPath +
  ...
  nodes.forEach(node => {
    if (node.getText().indexOf('routes') > -1 && node.getChildCount() > 0) {
      ...
      const start = openParenthsList[2].pos;
      const content =
        _buildIndent(4) +
...
export function findRoutingModule(host: Tree, generateDir: string): Path {
  ...
  while (dir) {
    const matches = dir.subfiles.filter(p => {
      return routingModuleRe.test(p);
    });
